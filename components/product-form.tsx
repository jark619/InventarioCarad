...
export function ProductForm({ onSaved }: { onSaved?: () => void }) {
  ...
  async function submit(e: FormEvent<HTMLFormElement>) { e.preventDefault(); setSaving(true); setError(''); const data = new FormData(e.currentTarget); const client = supabase(); let image_url: string | null = null;
    ...
    const { error } = await client.from('products').insert({ name: data.get('name'), barcode: data.get('barcode') || null, quantity: Number(data.get('quantity')), category: data.get('category') || null, image_url, price: Number(data.get('price')), low_stock_threshold: Number(data.get('threshold') || 5) });
