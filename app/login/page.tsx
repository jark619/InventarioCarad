...
export default function LoginPage() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) { event.preventDefault(); setLoading(true); setMessage(''); const form = new FormData(event.currentTarget); const email = String(form.get('email')); const password = String(form.get('password'));
    ...
    setLoading(false); if (result.error) return setMessage(result.error.message); if (mode === 'signup' && !result.data.session) return setMessage('Revisa tu correo para confirmar la cuenta.'); if (result.data.session) await supabase().auth.setSession(result.data.session); location.assign('/'); }
