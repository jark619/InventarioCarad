...
export default function PricingPage() {
  ...
  return <main className="min-h-screen bg-slate-50">
    <section className="mx-auto max-w-5xl px-5 py-12 text-center">
      <p className="text-sm font-semibold text-blue-600">PLANES CLAROS, SIN SORPRESAS</p>
      <h1 className="mt-3 text-4xl font-bold tracking-tight">Elige cómo empezar a vender</h1>
      <p className="mt-3 text-slate-600">Activa inventario, caja y reportes en cuestión de minutos.</p>
    </section>
    <section className="mx-auto grid max-w-4xl gap-5 px-5 pb-8 md:grid-cols-2">
      {plans.map(plan => <article key={plan.id} className={`relative rounded-2xl border p-7 shadow-sm ${plan.featured ? 'border-blue-600 bg-blue-50 shadow-blue-100' : 'bg-white'}`}>
        {plan.featured && <span className="absolute -top-3 left-6 rounded-full bg-blue-600 px-3 py-1 text-xs font-semibold text-white">Más elegido</span>}
    ...
    <p className="pb-10 text-center text-sm text-slate-500">Pago seguro con Stripe · Cancela cuando lo necesites</p>
  </main>;
}
